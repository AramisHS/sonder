...
export default function CashRegisterClosing() {
  ...
  const fetchClosings = async () => {
    setLoading(true);
    try {
      const { data: closingsData } = await supabase.from('cash_register_closings').select('*').order('closing_date', { ascending: false }).limit(1000);
      const { data: profilesData } = await supabase.from('profiles').select('id, full_name');
      const profileMap = Object.fromEntries((profilesData ?? []).map(p => [p.id, p]));
      const enriched = (closingsData ?? []).map(c => ({ ...c, profiles: c.created_by ? profileMap[c.created_by] || null : null }));
  ...
  const checkAlreadyClosed = async () => {
    const { data } = await supabase.from('cash_register_closings').select('id').eq('closing_date', closingDate).maybeSingle();
  ...
  const handleClosing = async () => {
    ...
    const { data: sales } = await supabase.from('sales').select('total, payment_method').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());
    ...
    const { data: created } = await supabase.from('cash_register_closings').insert({
    ...
    if (created) await logAudit(profile.id, 'INSERT', 'cash_register_closings', created.id, null, created);
    setSaving(false);
    setNotes('');
    fetchClosings();
    checkAlreadyClosed();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '1rem' }}>
      ...
      <div style={{ padding: '1.25rem', border: '1px solid #edf2f7', borderRadius: '1rem', background: '#ffffff', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ flex: 1, minWidth: '150px' }}>
            ...
            <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Observaciones del día..." className="input" />
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            {alreadyClosed ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '0.5rem', background: '#fef3c7', color: '#92400e', fontSize: '0.875rem' }}>
                <AlertCircle style={{ width: '1rem', height: '1rem' }} /> Ya existe corte
              </div>
            ) : (
              <button onClick={handleClosing} disabled={saving} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '0.5rem', background: '#0b3b4c', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 500, fontSize: '0.875rem' }}>
                {saving && <Loader2 style={{ width: '1rem', height: '1rem', animation: 'spin 1s linear infinite' }} />}Realizar corte
              </button>
            )}
          </div>
        </div>
            ...
            <table className="table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>Total ventas</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>Efectivo</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>Transferencia</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>Tarjeta</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>No. Ventas</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1 }}>Responsable</th>
                  <th style={{ position: 'sticky', top: 0, background: '#f8fafc', zIndex: 1, textAlign: 'right' }}>PDF</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  paginatedItems.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        <button onClick={() => handlePDF(c)} style={{ padding: '0.375rem', borderRadius: '0.5rem', border: 'none', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>
